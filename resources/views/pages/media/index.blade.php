...
@section('content')
    ...
    <section id="media">
        <form id="media-form" action="{{ route('media.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <button id="add-itinerary" class="btn btn-primary @error('image') is-invalid @enderror" onclick="$('#media-input').click()" type="button">
        ...
        <div class="mt-4 mx-2">
            <div class="row">
                @foreach ($media as $m)
                    <button class="media__btn btn col-12 col-sm-6 col-md-4 col-lg-3 p-2" data-id="{{ $m->id }}" data-toggle="modal" data-target="#media-edit-modal" type="button">
                        <div class="overflow-hidden">
                            <img class="media-display__img img-fluid" src="{{ $m->thumbnail_url }}" alt="{{ $m->name }}" />
                        </div>
                    </button>
                @endforeach
            </div>
